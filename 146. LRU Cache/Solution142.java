...
public class Solution142 {
    public static void runLRU(LRUCache lru, String[] command, int[][] values, Integer[] expected) {
        for (int idx = 1; idx < command.length; idx++) {
            if (command[idx].equals("get")) {
                Integer currentValue = lru.get(values[idx][0]);
                if (!Objects.equals(expected[idx], currentValue)) {
                    System.out.println("FAILURE");
                }
            }
            if (command[idx].equals("put")) {
    ...
    public static void main(String[] args) {
        ...
        int capacity = values[0][0];
        LRUCache lru = new LRUCache(capacity);
        runLRU(lru, commands, values, expected);
    }
}
